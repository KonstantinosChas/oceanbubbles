...
disp('did you change all the parameters?');
disp('which phase? (peak,trough,posp2,minp2)');
phase = input('','s');
name =['JS_16_',phase];   % name structure of images. images in folder are numbered after name (e.g. ..trough_0001)
name_0 = ['JS_16_',phase,'_000'];
name_10 = ['JS_16_',phase,'_00'];
name_100 = ['JS_16_',phase,'_0'];
name_1000 = ['JS_16_',phase,'_'];
% when running with no display
set(0,'DefaultFigureVisible','on'); % no graphics/plots
% choose which images are going to be loaded and analysed 
NUM_in = 1300;   % choose first image index (check file names in folder)
NUM = 1300;    %   choose last image index
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
date = '300621';
wave = 'JS_16';
stage = 'entire_video';
...
%     C:\Users\Konstaninos\OneDrive - University College London\Desktop\Work\third year\laboratory\bubbles\data\GW_175\191020\server\entire_video\processed_images\ROI4\run1
    
    dir_save = ['C:\Users\Konstaninos\OneDrive - University College London\Desktop\Work\third year\laboratory\bubbles\data\'...
        ,wave,'\',date,'\server\',stage,'\processed_images\',ROI,'\',run,'\'];
elseif T==1
    %%%linux
...
%     disp(['image calibrated ',num2str(i)]);
    cd(dir_save);
    figure(1); imshow(images.(im_name{i}));
    export_fig([dir_save, ['raw_',im_name{i}]],'-png','-nocrop'); 
...
%--------------------------------------------------------------------------
    cd(dir_save);
    figure(2); imshow(f2.(im_name{i}));
    export_fig([dir_save, ['preprocess_',im_name{i}]],'-png','-nocrop'); 
%--------------------------------------------------------------------------
...
%  sensitivity: 0.9,0.95,0.95,0.90
    clear rmax_xs rmin_xs
    clear radii 
    clear centers
    rmin_xs = 1; % in pixels
    rmax_xs= 30;
    centers.(im_name{i}) = struct();
    radii.(im_name{i}) = struct();
    [centers.(im_name{i}), radii.(im_name{i})] = hough_detect(rmin_xs,rmax_xs,f2.(im_name{i}));
%--------------------------------------------------------------------------
    cd(dir_save);
    figure(3); imshow(f2.(im_name{i}));
    hold on;
    viscircles(centers.(im_name{i}), radii.(im_name{i}),'Color','g');
    export_fig([dir_save, ['final_',im_name{i}]],'-png','-nocrop'); 
%--------------------------------------------------------------------------

% REMEMBER TO CHANGE RMIN-RMAX IN ANALYSIS!!
% Save files in allocated directory.
%     save(['radii.',im_name{i},'.mat'],'radii','-v7.3');
%     save(['centers.',im_name{i},'.mat'],'centers','-v7.3');
